...
| Resolve | `target` is auto-named; `image`/`storage_size` come from the source instance (or platform defaults if it is gone); the backup location, credentials, and S3 config come from platform convention. |
...
| `image` | `RESTORE_IMAGE` | | source | MariaDB image. Derived from the source instance; platform default if it is gone. |
...
- **Version sensitivity.** A physical restore must use the source's MariaDB
  version. When the source instance is gone and `image` is not provided, verify
  the platform default matches the backup, or pass `image` explicitly.
