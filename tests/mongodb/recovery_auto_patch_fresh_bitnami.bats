#!/usr/bin/env bats
# =============================================================================
# E2E proof that G1 self-heal's ConfigMap-auto-create (see
# recovery_auto_patch_fresh.bats) also works against a Bitnami-shaped
# layout — not just the official-image default this repo's other self-heal
# fixtures use. Self-heal's volume/mount/runAsUser detection is already
# profile-agnostic (it reads the live StatefulSet/mongod state, not a
# --profile flag), but nothing previously exercised that against a fixture
# that ALSO starts with the recovery ConfigMap missing entirely — this file
# closes that gap.
#
# Layout mirrors recovery_bitnami_profile.bats's fixture (volumeClaimTemplate
# "datadir" mounted at /bitnami/mongodb, dbpath nested two levels under that
# mount, runAsUser 1001, a small init-bitnami-dir init container that creates
# the nested dbpath dir on first boot) but, unlike that file, never calls
# setup-data-recovery.sh and never pre-creates the recovery ConfigMap — both
# must come from self-heal alone. Also proves the strategic-merge patch
# correctly coexists with the pre-existing init-bitnami-dir init container
# (initContainers merges by name, so self-heal's "data-recovery" entry must
# add alongside it, not replace it).
#
# Self-contained: setup_file creates the mongo-autopatch-fresh-bitnami
# namespace and all its resources. Reuses default object names so the
# existing aqsh-mongo-manager ClusterRole's resourceNames already cover the
# StatefulSet/secret; the ConfigMap is created by self-heal itself.
# teardown_file deletes the namespace. No other test file reads or writes
# mongo-autopatch-fresh-bitnami.
# =============================================================================
...
setup_file() {
  ...
  NS="mongo-core"
  FNS="mongo-autopatch-fresh-bitnami"
...
RB_EOF
  ...
  # Bitnami-shaped layout (see recovery_bitnami_profile.bats) — deliberately
  # NO data-recovery init container, and setup_file never creates the
  # recovery ConfigMap either: both halves of the One-Time Setup are
  # missing. The pre-existing init-bitnami-dir init container is there to
  # prove self-heal's strategic-merge patch coexists with it (merges by
...
STS_EOF
  ...
  echo "Waiting for 2-replica fresh-bitnami-autopatch RS rollout in ${FNS}..."
...
teardown_file() {
  ...
  kubectl --context "$ctx" delete namespace "mongo-autopatch-fresh-bitnami" --ignore-not-found 2>/dev/null || true
}
...
# ---------------------------------------------------------------------------
# Helpers — same pattern as recovery_auto_patch_fresh.bats.
# ---------------------------------------------------------------------------
...
# mongodb-1 as the safe, deterministic wipe target.
_init_rs() {
...
@test "Bitnami-shaped STS starts with neither the data-recovery init container nor the recovery ConfigMap" {
  ...
  [ "$status" -ne 0 ]
  # The pre-existing init-bitnami-dir init container must already be there —
  # otherwise this isn't testing what it claims to.
  run bash -c "kubectl --context '$CTX_A' -n '$FNS' get statefulset mongodb -o jsonpath='{.spec.template.spec.initContainers[*].name}' | tr ' ' '\n' | grep -qx init-bitnami-dir"
  [ "$status" -eq 0 ]
...
@test "recovery/recover self-heals both the ConfigMap and the init container on a Bitnami-shaped StatefulSet, coexisting with the pre-existing init-bitnami-dir init container" {
  ...
  # The ConfigMap self-heal created is permanent, reusable state.
  run kubectl --context "$CTX_A" -n "$FNS" get configmap mongodb-recovery-config
  [ "$status" -eq 0 ]

  # The data-recovery init container patch is temporary — recover's own
  # internal reset step already reverted it. init-bitnami-dir, which was
  # never self-heal's to manage, must still be there untouched — proving
  # the strategic-merge patch/revert only ever touched the "data-recovery"
  # entry by name, never clobbering the pre-existing one.
  run bash -c "kubectl --context '$CTX_A' -n '$FNS' get statefulset mongodb -o jsonpath='{.spec.template.spec.initContainers[*].name}' | tr ' ' '\n' | grep -qx data-recovery"
  [ "$status" -ne 0 ]
  run bash -c "kubectl --context '$CTX_A' -n '$FNS' get statefulset mongodb -o jsonpath='{.spec.template.spec.initContainers[*].name}' | tr ' ' '\n' | grep -qx init-bitnami-dir"
  [ "$status" -eq 0 ]
  ...
  # Pod was genuinely recreated and wiped under uid 1001 at /bitnami/mongodb.
  local after_uid
