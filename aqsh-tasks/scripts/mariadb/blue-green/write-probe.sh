...
case "$PROBE_DATABASE" in
  '' | *[!A-Za-z0-9_]*)
    bg_fail "blue-green/write-probe" "database must match ^[A-Za-z0-9_]+$" "{}" 2
    ;;
esac
case "$PROBE_TABLE" in
  '' | *[!A-Za-z0-9_]*)
    bg_fail "blue-green/write-probe" "table must match ^[A-Za-z0-9_]+$" "{}" 2
    ;;
esac
case "$PROBE_ID" in
  '' | *[!0-9]*)
    bg_fail "blue-green/write-probe" "id must be an unsigned integer" "{}" 2
    ;;
esac
...
mariadb_sql "$primary" "$password" "INSERT INTO \`${PROBE_DATABASE}\`.\`${PROBE_TABLE}\` VALUES (${PROBE_ID}, $(bg_sql_string "$PROBE_NOTE")) ON DUPLICATE KEY UPDATE note = VALUES(note)" >/dev/null
