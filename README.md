...
| `create-account` | Create/recreate an account with database-scoped grants or guarded all-database read-only grants | [docs/mariadb/create-account.md](docs/mariadb/create-account.md) |
...
| `migration/preflight`, `/sourcedb-backup`, `/export-db-env-to-vault`, `/restore`, `/import-db-env-from-vault`, `/check-connection`, `/setup-replication` | Cross-cluster migration: backup via MinIO, credential relay via Vault, restore + replication on the target | [docs/mariadb/migration.md](docs/mariadb/migration.md) |
