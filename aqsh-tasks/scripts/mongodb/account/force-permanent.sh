...
FORCE_REASON="${FORCE_REASON:-MANUAL_FORCE_PERMANENT}"
...
policy_set=$(jq -nc \
  ...
  --arg forced_at "$now_utc" \
  --arg forced_by "$FORCED_BY" \
...
write_task_result "$(jq -n \
  --arg status "PERMANENT" \
  --arg reason_code "FORCED_PERMANENT" \
  --arg summary "account forced to permanent" \
  --arg namespace "$DB_NAMESPACE" \
  --arg auth_db "$ACCOUNT_AUTH_DB" \
  --arg username "$ACCOUNT_USERNAME" \
  --arg forced_by "$FORCED_BY" \
  --arg force_reason "$FORCE_REASON" \
  '{status:$status, reason_code:$reason_code, summary:$summary, namespace:$namespace, auth_db:$auth_db, username:$username, forced_by:$forced_by, force_reason:$force_reason}')"
