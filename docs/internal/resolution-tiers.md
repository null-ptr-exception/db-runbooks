...
| `replication/*` | config → auto-detect → fallback | `attach`, `status`, `detach`. Peer Service host is derived from the namespace; peer AQSH URL and TokenRequest minting are deploy-time / workload-identity only (never task inputs — contrast `blue-green/*`) | [cross-cluster-replication](../mariadb/cross-cluster-replication.md) |
