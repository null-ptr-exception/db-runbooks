...
if [[ -z "$MONGO_USER" ]] || [[ -z "$MONGO_PASS" ]]; then
  ...
  response_err "backup" "Credentials not found" > "$AQSH_RESULT_FILE"
  exit 1
fi
...
if kubectl -n "${DB_NAMESPACE}" exec "${POD_NAME}" -- \
  ...
  response_err "backup" "mongodump failed" > "$AQSH_RESULT_FILE"
  exit 1
fi
...
if kubectl -n "${DB_NAMESPACE}" cp "${POD_NAME}:/tmp/${BACKUP_ARCHIVE}" "${LOCAL_PATH}"; then
  ...
  kubectl -n "${DB_NAMESPACE}" exec "${POD_NAME}" -- rm -rf "/tmp/${BACKUP_NAME}"
  response_err "backup" "Copy failed" > "$AQSH_RESULT_FILE"
  exit 1
fi
