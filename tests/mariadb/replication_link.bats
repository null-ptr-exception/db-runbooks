...
#            → attach refuses  → rebuild → linked → assess (attach) → detach
#
...
@test "attach refuses to execute when a rebuild is required" {
  expect_task_reason "REBUILD_REQUIRED" "$AQSH_B_URL" "replication/attach" \
...
@test "rebuild dry run reports a plan without touching anything" {
  expect_task_ok "$AQSH_B_URL" "replication/rebuild" \
    "$(jq -nc --arg ns "$DB_NS" --arg tok "$TOKEN" '{namespace: $ns, peer_token: $tok}')"

  local data
  data="$(_task_result_data)"
  assert_equal "$(echo "$data" | jq -r '.changed')" "false"
  assert_equal "$(echo "$data" | jq -r '.stage')" "plan"

  # Still standing, still unlinked.
  run kubectl --context "$CTX_B" -n "$DB_NS" get mariadb mariadb -o jsonpath='{.spec.multiCluster.enabled}'
  assert_success
  refute_output "true"
}

@test "rebuild requires confirm" {
  expect_task_reason "INVALID_REQUEST" "$AQSH_B_URL" "replication/rebuild" \
    "$(jq -nc --arg ns "$DB_NS" --arg tok "$TOKEN" \
      '{namespace: $ns, peer_token: $tok, dry_run: "false"}')"
}

@test "rebuild re-seeds the standby and establishes replication" {
  # SLOW: physical backup on cluster-a, then destroy + restore + link on
  # cluster-b. This is the one test that exercises the destructive path.
  expect_task_ok "$AQSH_B_URL" "replication/rebuild" \
    "$(jq -nc --arg ns "$DB_NS" --arg tok "$TOKEN" \
...
@test "rebuild refuses to run on a standby that does not need it" {
  expect_task_reason "REBUILD_NOT_REQUIRED" "$AQSH_B_URL" "replication/rebuild" \
    "$(jq -nc --arg ns "$DB_NS" --arg tok "$TOKEN" \
      '{namespace: $ns, peer_token: $tok, dry_run: "false", confirm: "true"}')"
}
